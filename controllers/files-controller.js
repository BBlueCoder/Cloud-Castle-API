...
class FileController extends Controller {
  ...
  async addFiles() {
    ...
    if (!this.req.files)
      throw new EmptyBody(
        "No files found, please make sure to send files with naming the field 'files'"
      );
    ...
    for (const file of this.req.files) {
      ...
      if (result.rows.length > 0) addedFiles.push(result.rows[0]);
    }
  ...
  async getFileData() {
    const result = await _getFileFromDB.get(this)(
      this.userId,
      this.req.params.fileId
    );
  ...
  async getFile() {
    ...
    const filePath = await _getStaticFilePath.get(this)(
      this.userId,
      fileMetaData.savedname
    );
  ...
  async getThumbnail() {
    ...
    if (
      !fileMetaData.filetype.includes("image") &&
      !fileMetaData.filetype.includes("video")
    )
  ...
  async deleteFile() {
    ...
    if (result.rows.length < 1) throw new FileNotFound();
  ...
  async deleteFiles() {
    if (!this.req.body[0])
      throw new EmptyBody(
        "No files found, please make sure to send a json body that contains ids of files"
      );

    const removedFiles = [];
    const key = `files-removed-${this.userId}`;
    const cacheRemovedFiles = await redisAPI.get(key);
    if (cacheRemovedFiles) removedFiles.push(cacheRemovedFiles);
